...
func _run() -> void:
	...
	await _settle(panel)
	_check(panel.get("selected").size() == 64 and not workspace.has_unsaved_changes() and undo.has_undo(), "selection did not stay transient or enter shared Undo")
	...
	undo.undo()
	_check(resource.voxels == original and undo.has_undo() and panel.get("selected").is_empty(), "Undo did not restore source while retaining earlier mask history")
	undo.redo()
