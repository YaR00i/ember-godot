...
func _run() -> void:
	...
	check(panel.find_child("VoxelLibraryNewLargeTree", true, false) is Button, "large tree object entry is missing")
	check(panel._place_button != null and not panel._place_button.disabled, "primary placement action is unavailable")
	check(panel._edit_button != null and not panel._edit_button.disabled, "direct template edit action is unavailable")
	...
	check(panel._edit_button.get_global_rect().end.y <= panel.get_global_rect().end.y + 1.0, "template edit action is clipped at compact height")
	...
	check(panel._edit_button.text == "Редактировать шаблон" and panel._edit_button.tooltip_text.contains("все"), "native template edit is not explicit about shared instances")
	...
	if not legacy_id.is_empty():
		panel._apply_filter(legacy_id)
		check(panel._edit_button.text == "Перенести и редактировать", "legacy template edit does not expose migration")
	(panel._filter_buttons["all"] as Button).pressed.emit()
	await process_frame
	...
	panel._edit_button.pressed.emit()
	check(edited_id == panel._picker.selected_id(), "template edit action does not emit the selected canonical model ID")
	...
	check(panel._edit_button.get_global_rect().end.x <= panel.get_global_rect().end.x + 1.0, "template edit action overflows at design width")
