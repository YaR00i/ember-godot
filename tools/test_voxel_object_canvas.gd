...
func _run() -> void:
	...
	var rename_edit = session(a, scene, undo)
	check(rename_edit.target_name() == "A", "session does not expose the selected scene-object name")
	check(not rename_edit.rename_target("").ok and a.name == "A", "empty object name was accepted")
	check(not rename_edit.rename_target("Bad/Name").ok and a.name == "A", "invalid object name was accepted")
	check(not rename_edit.rename_target("B").ok and a.name == "A", "duplicate sibling object name was accepted")
	check(rename_edit.rename_target("RenamedCanvasObject").ok and a.name == "RenamedCanvasObject", "object rename was not applied")
	undo.undo()
	check(a.name == "A", "object rename Undo failed")
	undo.redo()
	check(a.name == "RenamedCanvasObject", "object rename Redo failed")
	var renamed_scene := PackedScene.new()
	check(renamed_scene.pack(scene) == OK, "renamed scene pack failed")
	var renamed_scene_path := fixture.path_join("renamed_scene.tscn")
	check(ResourceSaver.save(renamed_scene, renamed_scene_path) == OK, "renamed scene save failed")
	var reopened_renamed := (load(renamed_scene_path) as PackedScene).instantiate()
	check(reopened_renamed.get_node_or_null("RenamedCanvasObject") != null, "save/reopen lost object rename")
	reopened_renamed.free()
	undo.undo()
	check(a.name == "A", "object name was not restored after rename gate")
	...
	canvas.open_object(session(a, scene, undo))
	check(canvas._object_name_input.editable and canvas._object_name_input.text == "A", "Canvas header does not show editable scene-object name")
	canvas._object_name_input.text = "CanvasHeaderName"
	canvas._commit_object_name()
	check(a.name == "CanvasHeaderName" and canvas._object_name_input.text == "CanvasHeaderName", "Canvas header rename did not reach the scene object")
	undo.undo()
	canvas._update_workshop_shell()
	check(a.name == "A" and canvas._object_name_input.text == "A", "Canvas header did not follow rename Undo")
