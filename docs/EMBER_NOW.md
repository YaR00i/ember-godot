...
- По просьбе пользователя подготовлена временная передача **только UI** в новый
  чат. [План UI](EMBER_EDITOR_UI_REWORK_PLAN.md) фиксирует направление и приёмку;
  сначала read-only ориентация и согласование макета, затем реализация лично
  новым координатором без субагентов. Основной чат остаётся для возвращения.
  UI штампов не принят: пользователь не смог найти инструмент. Новые функции
  мастерской приостановлены. Большой план сохранён в EMBER_PRODUCT_PLAN.md.
  Git-строка выше обозначает исторический gameplay baseline, не текущий HEAD;
  точный checkpoint передачи проверять через git log/status.

- Одиночные объёмные штампы реализованы, ручная приёмка открыта. Canvas →
  «ОБЪЁМНЫЕ ШТАМПЫ»: выделение → название → сохранить; выбрать пресет →
  «Разместить штамп…». ЛКМ ставит опору, стрелки/числа уточняют положение,
  ...
  Targeted stamp + selection interaction/fragment/merge/object canvas прошли;
  native Forward+ capture проверен. 16384 / 131072 cells: plan ~60ms, без remesh
