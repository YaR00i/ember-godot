...
- Контентный проход главы завершён примерно на две трети. Зафиксированы сцена
  спасения, обе дороги в комплекс, общий порядок помещений, обычный бой,
  исследователь, ограниченное разделение пары, водная и воздушно-тепловая
  головоломки, авторские разрушаемые секции, многоклеточные противники с частями
  тела и интерактивные объекты боя. Эти новые правила пока являются дизайном:
  runtime, Resources и schema под них ещё не менялись.
...
Следующий проход завершает проектирование канонического graybox: пространственную
схему связанной карты, точную конфигурацию арены и функциональных частей стража,
квестовые флаги и выгоду третьей улики, обязательные диалоговые состояния и
минимальный перечень graybox-assets/UI-состояний. После этого согласуется
отдельный технический Task Contract реализации.

До канонического боя парой нужен отдельный контракт на активную группу из 1–4
героев и её сохранение без второго party/save owner. Парный auto-approach
...
Основной Codex-thread теперь ведёт текущий milestone как координатор, а
согласованные реализации последовательно передаются одному пишущему worker.
