...
- В Codex сервер глобально отключён и включается только project-scoped override
  `.codex/config.toml` доверенного `ember-godot`; Cursor продолжает использовать
  собственную конфигурацию. Оба пути запускают `tools/godot-mcp.cmd`; supervisor
  обязан завершать всё дерево `npx` при закрытии stdio или родительской задачи.
  Не заменять этот путь прямым глобальным `npx`, иначе на Windows дочерние
  Node-процессы могут пережить закрытую задачу и занять порты `6505–6514`.
