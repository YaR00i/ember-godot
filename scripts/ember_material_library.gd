...
const PARAMETERS := {
	...
	"highlight_color": TYPE_COLOR, "highlight_color_strength": TYPE_FLOAT,
	"iridescence_strength": TYPE_FLOAT, "iridescence_frequency": TYPE_FLOAT,
	"texture_kind": TYPE_INT, "texture_scale": TYPE_FLOAT,
	"texture_strength": TYPE_FLOAT, "texture_relief": TYPE_FLOAT,
}
const RANGES := {
	...
	"highlight_color_strength":Vector2(0.0,1.0), "iridescence_strength":Vector2(0.0,1.0),
	"iridescence_frequency":Vector2(0.5,6.0), "texture_scale":Vector2(0.2,16.0),
	"texture_strength":Vector2(0.0,0.3), "texture_relief":Vector2(0.0,0.05),
}
...
static func validate_options(options: Dictionary) -> PackedStringArray:
	...
	for parameter in options:
		...
		elif expected == TYPE_INT and (value < 0 or value > 3):
			errors.append("Out of range: " + parameter)
